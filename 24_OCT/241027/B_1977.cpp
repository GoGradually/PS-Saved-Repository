...
void Solve() {
    ...
    for (int i = 0; i < arr.size(); i++) {
        if (!temp.empty() && temp.back().first == arr[i])
            temp.back().second++;
        else
    ...
    vector<int> ans;
    bool fillZeroMode = true;
    for (int i = 0; i < temp.size(); i++) {
        if (fillZeroMode && temp[i].first == 1 && temp[i].second > 1) {
            ans.push_back(-1);
            temp[i].second--;
            fillZeroMode = false;
        } else if (!fillZeroMode && temp[i].first == 0 && temp[i].second > 1) {
            fillZeroMode = true;
            temp[i].second--;
            ans.push_back(1);
        }
