...
void Solve() {
    ...
    cin >> n >> k;
    vector<int> arr(2 * n);
    vector<int> appearA(n + 1), appearB(n + 1);
    for (int i = 0; i < 2 * n; i++) {
        cin >> arr[i];
    }
    ...
    queue<int> aAppearDouble, bAppearDouble;
    queue<int> appearOnce;
    for (int i = 1; i <= n; i++) {
        if (appearA[i] == 1) {
            appearOnce.push(i);
        } else if (appearA[i] == 2) {
            aAppearDouble.push(i);
        } else if (appearB[i] == 2) {
            bAppearDouble.push(i);
        }
    }
    k *= 2;
    vector<int> aAns, bAns;
    while (k > 0) {
        if (k >= 2 && !aAppearDouble.empty()) {
            k -= 2;
            aAns.push_back(aAppearDouble.front());
            aAns.push_back(aAppearDouble.front());
            aAppearDouble.pop();
            bAns.push_back(bAppearDouble.front());
            bAns.push_back(bAppearDouble.front());
            bAppearDouble.pop();
            continue;
        }
        aAns.push_back(appearOnce.front());
        bAns.push_back(appearOnce.front());
        appearOnce.pop();
        k--;
    }
