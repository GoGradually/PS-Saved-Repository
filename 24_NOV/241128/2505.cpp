...
bool goLeft(vector<int>& arr, int cnt, int beforeL, int beforeR);
bool goRight(vector<int>& arr, int cnt, int beforeL, int beforeR);
...
bool goLeft(vector<int>& arr, int cnt, int beforeL, int beforeR) {
    ...
    for (int i = 0; i < arr.size(); i++) {
        if (i != arr[i]) {
            ...
            if (validate(temp)) {
                cout << beforeL << ' ' << beforeR << '\n';
                cout << l + 1 << ' ' << r + 1 << '\n';
                return true;
            }

            if (goRight(temp, cnt + 1, l + 1, r + 1)) {
                return true;
            }
            return false;
        }
