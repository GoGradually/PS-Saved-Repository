...
void Solve() {
    ...
    vector<vector<vector<int>>> attack(
        61, vector<vector<int>>(61, vector<int>(61, -1)));
    attack[arr[0]][arr[1]][arr[2]] = 0;
    queue<tuple<int, int, int>> bfsQ;
    bfsQ.push({arr[0], arr[1], arr[2]});
    while (!bfsQ.empty()) {
        int x, y, z;
        tie(x, y, z) = bfsQ.front();
        bfsQ.pop();
        vector<int> perm(3);
        perm[0] = 1;
        perm[1] = 3;
        perm[2] = 9;
        do {
            int p = x - perm[0] >= 0 ? x - perm[0] : 0;
            int q = y - perm[1] >= 0 ? y - perm[1] : 0;
            int r = z - perm[2] >= 0 ? z - perm[2] : 0;
            if (attack[p][q][r] == -1) {
                bfsQ.push({p, q, r});
                attack[p][q][r] = attack[x][y][z] + 1;
            }
