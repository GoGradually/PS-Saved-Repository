...
int bfs(vector<vector<int>>& arr, vector<pair<int, int>>& can,
        vector<int>& canArr) {
    int ret = 0;
    int n = arr.size();
    ...
    while (!bfsQ.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            bfsQ.push({p, q});
            ret = max(ret, check[p][q]);
        }
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (arr[i][j] != 1 && check[i][j] == -1) return INF;
        }
    }
...
void Solve() {
    ...
    vector<int> canArr(can.size(), 0);
    for (int i = 0; i < m; i++) {
        canArr[i] = 1;
    }
    int ans = INF;
    do {
        ans = min(ans, bfs(arr, can, canArr));
    } while (prev_permutation(canArr.begin(), canArr.end()));
    if (ans == INF) ans = -1;
    cout << ans << '\n';
}
...
1. 몇초만에 걸리는가 -> N^2 -> 2500
2. 그래서 몇개만에 가능? 10C5 -> 252
630000
