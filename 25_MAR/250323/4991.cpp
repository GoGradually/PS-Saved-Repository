...
void bfs(pair<int, int> start, vector<vector<int>> &check,
         vector<string> &arr) {
    ...
    while (!bfsQ.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            if (p >= arr.size() | q >= arr[0].size() | p < 0 | q < 0) continue;
            if (arr[p][q] == 'x') continue;
...
void Solve(int n, int m) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (arr[i][j] == 'o') robot = {i, j};
            if (arr[i][j] == '*') dust.push_back({i, j});
        }
    }

    vector<vector<int>> robotCheck(n, vector<int>(m, -1));
    vector<vector<vector<int>>> dustCheck(
        dust.size(), vector<vector<int>>(n, vector<int>(m, -1)));
    bfs(robot, robotCheck, arr);
    for (int i = 0; i < dust.size(); i++) bfs(dust[i], dustCheck[i], arr);

    vector<int> permutate(dust.size());
    ...
    do {
        int dist = 0;
        for (int i = 0; i < permutate.size() - 1; i++) {
            int now = permutate[i];
            int next = permutate[i + 1];
            dist += dustCheck[now][dust[next].first][dust[next].second];
        }
        dist += robotCheck[dust[permutate[0]].first][dust[permutate[0]].second];
        ans = min(ans, dist);
    } while (next_permutation(permutate.begin(), permutate.end()));

    for (int i = 0; i < dust.size(); i++) {
        if (dustCheck[i][robot.first][robot.second] == -1) {
            cout << -1 << '\n';
            return;
        }
    }
...
*/
// 알고리즘의 작동방식 "완전히" 이해하려 노력하기 -> 수식 & 그림화, 조건
// 정리 수행 목표
// 1. 아이디어 획득 : "추론"
...
//     - 뚜렷한 최적화 기법이 떠오르지 않을 땐, 문제에 주어진 특이한 특징을
//     잡기
//     - 단위 동작의 조건 분기 파악
