...
public class A_1993 {
    ...
    public static void run(BufferedReader br, BufferedWriter bw) throws IOException{
        ...
        List<Integer> arr = new ArrayList<>(Arrays.asList(0, 0, 0, 0));
        for (int i = 0; i < n*4; i++) {
            if(line.charAt(i) == '?') continue;
            int now = (int)(line.charAt(i) - 'A');
            arr.set(now, arr.get(now) + 1);
        }
        int ans = 0;
        ans += Math.min(arr.get(0), n);
        ans += Math.min(arr.get(1), n);
        ans += Math.min(arr.get(2), n);
        ans += Math.min(arr.get(3), n);  
