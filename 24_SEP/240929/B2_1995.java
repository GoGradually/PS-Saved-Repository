...
public class B2_1995 {
    ...
    public static void run(BufferedReader br, BufferedWriter bw) throws IOException{
        ...
        st = new StringTokenizer(line);
        for (int i = 0; i < n; i++) {
            mp.put(arr.get(i), Long.valueOf(st.nextToken()));
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(first >= m){
                Long second = m / a * a;
                ans = Math.max(ans, Math.min(m, second + Math.min(m/a,bCnt)));
            }else{
                Long remain = m - a*aCnt;
                Long bWant = remain / b;
                Long bCost = Math.min(bWant, bCnt);
                ans = Math.max(ans,
                    Math.min(m, a*aCnt + b*bCost 
                    + Math.min(bCnt - bCost, aCnt))
                );
