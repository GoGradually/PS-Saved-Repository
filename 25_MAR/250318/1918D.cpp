...
void Solve() {
    ...
    auto check = [&](ll val) {
        vector<ll> dp(n + 1);
        set<pair<ll, int>> minimum;
        minimum.insert({0, 0});
        for (int i = 1; i <= n; i++) {
            while (!minimum.empty() &&
                   psum[i - 1] - psum[minimum.begin()->second] > val) {
                minimum.erase(minimum.begin());
            }
            if (minimum.empty()) return false;
            dp[i] = minimum.begin()->first + arr[i];
            // cout << val << ' ' << i << ": " << dp[i] << '\n';
            minimum.insert({dp[i], i});
        }
        ll minVal = lINF;
        for (int i = 0; i <= n; i++) {
            minVal = min(minVal, max(dp[i], psum[n] - psum[i]));
        }
