...
void Solve() {
    ...
    vector<vector<int>> roomNumber(n, vector<int>(m));
            ...
            if (check[i][j] == -1) {
                ...
                while (!bfsQ.empty()) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        if (check[p][q] != -1) {
                            if (check[p][q] != check[x][y]) {
                                nearRooms.push_back(check[p][q]);
                            }
                            continue;
                        }
                ...
                roomSize[roomCnt] = nowRoomSize;
                maxRoomSize = max(maxRoomSize, nowRoomSize);
                for (int k = 0; k < nearRooms.size(); k++) {
                    breakRoomSize = max(breakRoomSize, roomSize[nearRooms[k]] +
                                                           roomSize[roomCnt]);
                }
            }
        }
