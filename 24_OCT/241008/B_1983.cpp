...
void Solve() {
    ...
    vector<vector<int> > a_(n, vector<int>(m)), b_(n, vector<int>(m));
...
같은 위상 안에 있는가?
위, 죄 모서리는 오른쪽 아래의 종속변수
위, 좌 를 제외한 모든 부분을 0으로 만들었을 때, 두 모서리가 같은가?
위, 좌를 제외한 좌표 만들기를 진행했을 때, 두 배열이 같아지는가?
