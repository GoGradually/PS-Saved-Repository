...
void Solve() {
    ...
    for (int i = 0; i < arr.size(); i++) {
        if (arr[i].second < k) {
            if (arr[i].first == arr.back().first &&
                arr[i].second + arr.back().second == k) {
            ...
            if (arr[i].first == arr.back().first) {
                if (arr[i].second + arr.back().second == 2 * k) {
                    for (int j = i + 1; j < arr.size() - 1; j++) {
                        if (arr[j].second != k) {
                            cout << -1 << '\n';
                            return;
