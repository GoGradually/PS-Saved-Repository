...
void Solve() {
    ...
    for (int i = 0; i < q; i++) {
        int x;
        cin >> x;
        int index = upper_bound(left.begin(), left.end(), x) - left.begin() - 1;
        int l = min(left[index] + right[index] - x, x);
        int r = max(left[index] + right[index] - x, x) + 1;
        imos[l]++;
        imos[r]--;
    }
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (result[i] % 2 == 1) {
            int index =
                upper_bound(left.begin(), left.end(), i) - left.begin() - 1;
            int reversed = left[index] + right[index] - i;
            cout << str[reversed - 1];
        } else {
            cout << str[i - 1];
        }
