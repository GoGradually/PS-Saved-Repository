...
map<int, int> numberCardSet;
vector<tuple<int, int, int>> playerCommandCardSet;

int useCard(int nowPlayer, tuple<int, int, int> nowCard) {
    int id, command, num;
    tie(id, command, num) = nowCard;

    if (command == 1) {
        if (numberCardSet[num] != 0) {
            playerCommandCardSet[nowPlayer] = nowCard;
        } else {
            numberCardSet[num] = nowPlayer;
        }
    } else if (command == 2) {
        numberCardSet[num] = 0;
    }
...
void Solve() {
    int n, t;
    cin >> n >> t;
    playerCommandCardSet =
        vector<tuple<int, int, int>>(n + 1, make_tuple(0, 0, 0));
    queue<int> playerTurn;
    for (int i = 0; i < t; i++) {
        int temp;
        cin >> temp;
        playerTurn.push(temp);
    }
    queue<tuple<int, int, int>> card;
    for (int i = 0; i < t; i++) {
        string str;
        int id, num = 0;
        cin >> id >> str;
        int command;
        if (str == "next") command = 0;
        if (str == "acquire") {
            command = 1;
            cin >> num;
        }
        ...
        card.push({id, command, num});
    }

    while (!playerTurn.empty()) {
        int nowPlayer = playerTurn.front();
        playerTurn.pop();
        tuple<int, int, int> nowCard;
        if (playerCommandCardSet[nowPlayer] != make_tuple(0, 0, 0)) {
            nowCard = playerCommandCardSet[nowPlayer];
            playerCommandCardSet[nowPlayer] = {0, 0, 0};
        } else {
            nowCard = card.front();
            card.pop();
        }
        cout << useCard(nowPlayer, nowCard) << '\n';
    }
}
